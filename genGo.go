	...
	if v.Union && len(v.MemberTypes) > 0 {
		if _, ok := gen.StructAST[v.Name]; !ok {
			...
			if fieldName != v.Name {
				...
				content += fmt.Sprintf("\tXMLName\txml.Name\t`xml:\"%s\" json:\"%s\"`\n", v.Name, v.Name)
			}
...
func (gen *CodeGenerator) GoComplexType(v *ComplexType) {
	if _, ok := gen.StructAST[v.Name]; !ok {
		...
		for _, attribute := range v.Attributes {
			...
			content += fmt.Sprintf("\t%sAttr\t%s\t`xml:\"%s,attr%s\" json:\"%s,attr%s\"`\n", genGoFieldName(attribute.Name, false), fieldType, attribute.Name, optional, attribute.Name, optional)
		}
		...
		if len(v.Base) > 0 {
			...
			if isGoBuiltInType(v.Base) {
				content += fmt.Sprintf("\tValue\t%s\t`xml:\",chardata\" json:\",chardata\"`\n", genGoFieldType(v.Base))
			} else {
				content += fmt.Sprintf("\t%s\n", genGoFieldType(v.Base))
			}
		}
...
func (gen *CodeGenerator) GoAttributeGroup(v *AttributeGroup) {
	if _, ok := gen.StructAST[v.Name]; !ok {
		...
		for _, attribute := range v.Attributes {
			...
			content += fmt.Sprintf("\t%sAttr\t%s\t`xml:\"%s,attr%s\" json:\"%s,attr%s\"`\n", genGoFieldName(attribute.Name, false), genGoFieldType(getBasefromSimpleType(trimNSPrefix(attribute.Type), gen.ProtoTree)), attribute.Name, optional, attribute.Name, optional)
		}
		content += "}\n"
		gen.StructAST[v.Name] = content
		gen.Field += fmt.Sprintf("%stype %s%s", genFieldComment(fieldName, v.Doc, "//"), fieldName, gen.StructAST[v.Name])
